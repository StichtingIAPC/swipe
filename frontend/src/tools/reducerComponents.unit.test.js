...
describe('reducerComponent should run correctly with a basic implementation of such a reducer for component: ', () => {
	...
	test('setFieldReducer ', () => {
		...
		let action = {type: actionOne, field1: 21};
		expect(setFieldReducerInstance(undefined, action)).toBe(21);

		action = {type: "WRONG", field1: 21};
		...
		let action = {type: actionOne, field: 'test', value: 21};
		expect(setFieldReducerInstance(undefined, action)).toEqual({test:21});

		action = {type: actionOne, field: 'potato', value: 'chips'};
		expect(setFieldReducerInstance({test:21}, action)).toEqual({test: 21, potato: 'chips'});

		action = {type: actionOne, field: 'test', value: 'chips'};
	...
	test('booleanControlReducer', () => {
		...
		const types = {[truthyAction.type]: true, [falsyAction.type]: false };
