 ...
 * @param path The path of the part of state, this reducer is responsible for. Example: '/logistics/externalise/'
...
/**
 * Generates all the action constants and functions you will likely need for CRUD.
 * @param path The path of the part of state, this reducer is responsible for. Example: '/logistics/externalise/'
 * @returns see crudActions and crudFunctions
 */
export function crudObjects(path) {
	return {
		...crudActions(path),
		...crudFunctions(path),
	};
 ...
 * @param customReducers if you want to have custom reducers for the `singleName` or `manyName` then put them here.
 * Also any other reducers for this part of that can be here.
 * @param overrideMethod By default, it will first run the generated reducer and then your custom reducer.
 * You can change this by supplying your own function here. It's parameters will be the two colliding objects and its return will be placed at the resulting object.
 * @returns CRUDReducers CRUD Reducers
 */
export function crudReducers(path, singleName, manyName, customReducers, overrideMethod = collectReducers) {
	if (!path) {
	...
	if (customReducers) {
		...
		generatedReducers = mergeObjects(generatedReducers, customReducers, overrideMethod);
	}
 ...
 * @returns {Function}
 */
export function crudSagas(path, api, baseSelector, manyName = 'items', singleName = 'currentItem') {
	const actions = crudObjects(path);
