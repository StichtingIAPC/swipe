...
class LabelTypeEdit extends React.Component {
	...
	getResetState(props = this.props) {
		if (props.labelType != null) return {...props.labelType};
		return {id: null, name: '', description: '', unit_type: null, labels: []};
	}
	...
	render() {
		return (
			<Form
				...
				<StringField onChange={(evt) => this.setState({name: evt.target.value})} value={this.state.name} name="Name" />
				<StringField onChange={(evt) => this.setState({description: evt.target.value})} value={this.state.description} name="Description" />
				<SelectField disabled={!!this.state.id} onChange={(evt) => this.setState({unit_type: Number(evt.target.value)})} value={this.state.unit_type || 1} name="Unit type" options={this.props.unitTypes} nameField="type_long" />
			</Form>
		)
	}
}
...
export default connect(
	(state, props) => ({
		...
		labelType: (state.labelTypes.labelTypes || []).find(el => el.id === Number(props.params.labelTypeID)),
