...
def remove_orderlines(stock_change: StockChange):
    ...
    :return:
    """
    acted_upon = [enum['cash_register'], enum['internalise'], enum['stock_count']]
    enum_val = stock_change.change_set.enum
    if stock_change.labeltype == OrderLabel.labeltype and  enum_val in acted_upon:
        if not stock_change.is_in:
            ...
            if len(lines) < orders_to_complete:
                action = ""
                if enum_val == enum['cash_register']:
